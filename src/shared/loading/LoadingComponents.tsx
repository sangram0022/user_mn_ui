...
export const AvatarSkeleton: React.FC<{ size?: 'sm' | 'md' | 'lg' }> = ({ size = 'md' }) => {
  ...
  return (
    <Skeleton className={sizes[size]} variant="circular" aria-label="Loading profile picture" />
  );
};
...
export const CardSkeleton: React.FC<CardSkeletonProps> = ({
  ...
  return (
    <div className={`p-4 border border-gray-200 rounded-lg space-y-4 ${className}`}>
      {hasImage && <Skeleton height="200px" variant="rounded" aria-label="Loading image" />}
      ...
      {Array.from({ length: items }, (_, index) => (
        ...
        >
          ...
          <div className="flex-1 space-y-1">
            ...
            {hasSecondaryText && <Skeleton height="0.875rem" width="40%" variant="text" />}
          </div>
...
export function useLoadingState(initialState: Partial<LoadingState> = {}) {
  ...
  const startLoading = useCallback((text?: string, stage?: string) => {
...
export const LoadingSpinner: React.FC<LoadingSpinnerProps> = ({
  ...
  const colors = { primary: 'text-blue-600', secondary: 'text-gray-600', white: 'text-white' };
