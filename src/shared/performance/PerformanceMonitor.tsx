...
export function usePerformanceMonitor(config: PerformanceConfig = {}) { const {
  ...
  const metricsRef = useRef<PerformanceMetrics>({ renderTime: 0,
    componentMount: 0,
    memoryUsage: 0,
    interactionDelay: 0,
    coreWebVitals: {
  ...
  const trackRender = useCallback(() => {
    ...
    } else { // Fallback for browsers without scheduler
      setTimeout(() => {
        const renderTime = performance.now() - renderStartRef.current;
        metricsRef.current.renderTime = renderTime;
      }, 0);
  ...
  // Track memory usage
  const trackMemoryUsage = useCallback(() => { if (!trackMemory || !('memory' in performance)) return;
  ...
  useEffect(() => { if (!trackWebVitals) return;
    ...
    } catch (error) { logger.warn('Performance Observer not supported:', { error  });
    }
  ...
  const trackInteraction = useCallback((eventType: string) => {
  ...
  useEffect(() => { if (!onMetricsUpdate) return;
...
export function withPerformanceMonitoring<P extends object>(
  ...
  config: PerformanceConfig = {}
    ...
    const enhancedProps = { ...props,
      onPerformanceTrack: trackInteraction
    } as P;
...
export interface ProfilerProps { id: string;
...
export const PerformanceProfiler: React.FC<ProfilerProps> = ({ id, 
  ...
  const handleRender = useCallback((
    ...
    onRender?.(profileId, phase as 'mount' | 'update', actualDuration, baseDuration, startTime, commitTime);
  }, [onRender]);
