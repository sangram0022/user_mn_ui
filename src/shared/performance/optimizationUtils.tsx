 ...
 * Note: This file intentionally exports both components and utilities
 */
...
export class LazyLoadingManager {
  ...
  static createLazyComponent<P = object>(
    ...
    const LazyComponent = lazy(async () => {
      const moduleKey = importFunction.toString();
      ...
      const loadWithRetry = async (): Promise<{ default: T }> => {
        const attempts = this.retryAttempts.get(moduleKey) || 0;
        ...
        try {
          ...
          logger.error(
            `Failed to load module (attempt ${attempts + 1}):`,
            error instanceof Error ? error : new Error(String(error))
          );

          if (attempts < maxRetries) {
            this.retryAttempts.set(moduleKey, attempts + 1);
   ...
   */
  static preloadComponents(components: Array<{ preload: () => void }>): void {
...
export interface RouteConfig<P = object> {
  path: string;
...
export class CodeSplittingManager {
  private static routeComponents = new Map<string, ComponentType>();
  private static preloadQueue: string[] = [];

  /**
   * Create route-based code splitting configuration
   */
  static createRouteComponents(routes: RouteConfig[]): Map<string, ComponentType> {
    routes.forEach((route) => {
      const LazyComponent = LazyLoadingManager.createLazyComponent(route.component, {
        fallback: (
   ...
   * Preload high-priority routes
   */
  private static preloadHighPriorityRoutes(): void {
    ...
    const preloadInIdle = () => {
      this.preloadQueue.forEach((path) => {
        ...
        if (
          ...
          typeof (component as { preload?: () => void }).preload === 'function'
        ) {
   ...
   */
  static getRouteComponent(path: string): ComponentType | undefined {
    return this.routeComponents.get(path);
  }
}

// ==================== MEMOIZATION UTILITIES ====================

export interface MemoOptions<P = object> {
...
export class MemoizationManager {
   ...
   */
  static memoComponent<P = object>(
    component: ComponentType<P>,
    options: MemoOptions<P> = {}
  ): MemoExoticComponent<ComponentType<P>> {
    const { equalityFn, displayName } = options;
  ...
  private static deepEqual(obj1: unknown, obj2: unknown): boolean {
    ...
    for (const key of keys1) {
      ...
      if (!this.deepEqual(obj1Record[key], obj2Record[key])) return false;
    }
...
// Note: These are standalone hook functions, not class methods
    ...
    return (
      ...
      >
        <div style={{ paddingTop, paddingBottom }}>
          {visibleItems.map((item, index) => renderItem(item, visibleStart - overscan + index))}
        </div>
      </div>
    );
  }
...
export const OptimizedImage: React.FC<OptimizedImageProps> = memo(
  ({
    ...
    const { ref: imgRef, inView } = useInView({ threshold: 0.1, triggerOnce: true, skip: !lazy });
 ...
 */
export function useSmartTransition() {
...
/**
 * Batch multiple state updates using React transitions
 */
export function batchUpdatesWithTransition(updates: Array<() => void>): void {
  startTransition(() => {
    ...
    if (this.state.hasError) {
      return (
        <div className="p-4 border border-red-200 rounded-lg bg-red-50">
          <h3 className="text-red-800 font-semibold mb-2">Component Failed to Load</h3>
...
export const performanceUtils = {
  ...
  VirtualScroll,
  OptimizedImage,
  // Transition utilities are now standalone functions
  hooks: {
    useSmartTransition,
