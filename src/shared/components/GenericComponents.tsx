...
import { forwardRef, useMemo, useCallback, type ComponentProps, type ReactNode, type ElementType, type ComponentPropsWithRef, useEffect } from 'react';

// ==================== UTILITY TYPES ====================

export type PolymorphicRef<T extends ElementType> = ComponentPropsWithRef<T>['ref'];

export type PolymorphicComponentProp<T extends ElementType, P = object> = { as?: T; } & P & Omit<ComponentProps<T>, keyof P | 'as'>;

// ==================== GENERIC BUTTON COMPONENT ====================

export interface ButtonProps { variant?: 'primary' | 'secondary' | 'outline' | 'ghost' | 'danger';
  size?: 'sm' | 'md' | 'lg' | 'xl';
  loading?: boolean;
  disabled?: boolean;
  fullWidth?: boolean;
...
>(({ variant = 'primary',
  ...
  children,
  className = '',
  ...props }, ref) => { const buttonClasses = useMemo(() => {
    const baseClasses = [
...
export interface InputProps { label?: string;
  error?: string;
  helperText?: string;
  required?: boolean;
...
>(({ label,
  ...
  const inputClasses = useMemo(() => { const baseClasses = [
...
export interface CardProps { variant?: 'elevated' | 'outlined' | 'filled';
...
>(({ variant = 'elevated',
  ...
  const contentPadding = useMemo(() => { const paddingClasses = {
      none: '',
      sm: 'p-4',
      md: 'p-6',
      lg: 'p-8',
...
export const Modal: React.FC<ModalProps> = ({ isOpen,
  ...
  const modalSizes = useMemo(() => { const sizes = {
      sm: 'max-w-md',
      md: 'max-w-lg',
      lg: 'max-w-2xl',
      xl: 'max-w-4xl',
...
export interface Column<T> { key: keyof T;
  title: string;
...
export interface TableProps<T> { data: T[];
  columns: Column<T>[];
  loading?: boolean;
...
export function Table<T extends Record<string, any>>({ data,
  columns,
  loading = false,
  emptyState,
  onSort,
  ...
  onRowClick,
  striped = false,
  hoverable = true, }: TableProps<T>) { const handleSort = useCallback((key: keyof T) => {
    if (!onSort) return;
  ...
  const getRowKey = useCallback((item: T, index: number) => { if (typeof rowKey === 'function') {
      return rowKey(item, index);
    }
    if (rowKey && item[rowKey] !== undefined) { return String(item[rowKey]);
    }
    return index;
  }, [rowKey]);
