 ...
 * Deep required type
 */
export type DeepRequired<T> = { [P in keyof T]-?: T[P] extends object | undefined ? DeepRequired<NonNullable<T[P]>> : T[P]; };

/**
 * Non-nullable keys
 */
export type NonNullableKeys<T> = { [K in keyof T]: T[K] extends null | undefined ? never : K; }[keyof T];

/**
 * Nullable keys
 */
export type NullableKeys<T> = { [K in keyof T]: T[K] extends null | undefined ? K : never; }[keyof T];
 ...
 * Filter parameters
 */
export type FilterParams<T = Record<string, any>> = { [K in keyof T]?: T[K] | T[K][]; } & { search?: string;
  dateFrom?: string;
  dateTo?: string; };
 ...
 */
export type QueryParams<T = Record<string, any>> = PaginationParams & FilterParams<T>;
 ...
 * Form field state
 */
export type FormFieldState<T = any> = { value: T;
  error: string | null;
  touched: boolean;
  dirty: boolean;
  valid: boolean; };
 ...
 */
export type FormState<T extends Record<string, any>> = { [K in keyof T]: FormFieldState<T[K]>; };

/**
 * Form validation rules
 */
export type ValidationRule<T = any> = { required?: boolean;
  min?: number;
  max?: number;
  minLength?: number;
  maxLength?: number;
  ...
  asyncValidator?: (value: T, formData?: any) => Promise<string | null>; };

/**
 * Form validation schema
 */
export type ValidationSchema<T extends Record<string, any>> = { [K in keyof T]?: ValidationRule<T[K]>; };
...
export type WithHTMLAttributes<T = object> = T & 
  Pick<React.HTMLAttributes<HTMLElement>, 'id' | 'role' | 'aria-label' | 'aria-labelledby' | 'aria-describedby'>;
 ...
 */
export type Action<T = any> = { type: string;
  payload?: T;
 ...
 * Async state
 */
export type AsyncState<T = any> = { data: T | null;
  loading: boolean;
  error: string | null;
  lastFetch?: number; };
 ...
 */
export type ResourceState<T = any> = { items: T[];
  selectedItem: T | null;
  loading: boolean;
  error: string | null;
  pagination: {
 ...
 */
export type ThrottledFunction<T extends (...args: any[]) => any> = { (...args: Parameters<T>): ReturnType<T> | undefined;
  cancel(): void; };
 ...
 * Pick by value type
 */
export type PickByValue<T, U> = Pick<T, { [K in keyof T]: T[K] extends U ? K : never; }[keyof T]>;

/**
 * Omit by value type
 */
export type OmitByValue<T, U> = Pick<T, { [K in keyof T]: T[K] extends U ? never : K; }[keyof T]>;
 ...
 */
export function isObject(value: any): value is Record<string, any> { return value !== null && typeof value === 'object' && !Array.isArray(value); }
 ...
 */
export function isEmail(value: string): value is Email { const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  return emailRegex.test(value); }

/**
 * Type guard for checking if value is URL
 */
export function isURL(value: string): value is URL { try {
 ...
 */
export function createBrand<T, U extends string>(value: T): Brand<T, U> { return value as Brand<T, U>; }

/**
 * Assert that a value is of a specific type
 */
export function assertType<T>(value: any, guard: (value: any) => value is T): asserts value is T { if (!guard(value)) {
