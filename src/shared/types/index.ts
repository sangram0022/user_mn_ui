...
export interface UserRoleInfo { id?: number | string;
  name: string;
  description?: string;
  permissions?: string[]; }
...
export interface UserProfile extends User { last_login?: string | null;
  profile_data?: Record<string, unknown> | null; }

export interface UserSummary { id?: string | number;
  user_id: string;
  email: string;
  first_name: string;
  last_name: string;
...
export interface UserRole { id: number;
  name: string;
  description: string;
  permissions: string[]; }
...
export interface UpdateUserRequest { first_name?: string;
  last_name?: string;
  role?: string;
  is_active?: boolean;
  is_verified?: boolean;
...
export interface ChangePasswordRequest { current_password: string;
  new_password: string;
  confirm_password: string; }
...
export interface AuditLog { log_id: string;
  user_id: string;
  action: string;
  resource: string;
  resource_id: string;
...
export interface AuditSummary { total_logs: number;
  recent_actions: AuditLog[];
  security_events: number; }

export interface AuditLogsQuery { action?: string;
  resource?: string;
  user_id?: string;
  start_date?: string;
  end_date?: string;
...
export interface AdminUsersQuery { page?: number;
  limit?: number;
  role?: string;
  is_active?: boolean; }

export interface UserAnalytics { total_users: number;
  active_users: number;
  ...
  engagement_distribution?: { high: number;
    medium: number;
    low: number;
  };
  growth_rate?: number;
...
export interface LifecycleAnalytics { stages: Array<{
    stage: string;
    count: number;
    percentage: number;
  }>;
  transitions: Array<{ from_stage: string;
    to_stage: string;
    count: number;
  }>;
}

export interface CohortAnalysis { cohorts: Array<{
    period: string;
    size: number;
...
export interface WorkflowRequest { id: string;
  type: string;
  user_id: number;
...
export interface PendingWorkflow { id: string;
  type: string;
  description: string;
...
export interface PaginationProps { currentPage: number;
  totalPages: number;
  onPageChange: (page: number) => void;
  showFirstLast?: boolean;
  showPrevNext?: boolean; }

export interface TableColumn<T> { key: keyof T;
  label: string;
  sortable?: boolean;
  render?: (value: T[keyof T], row: T) => React.ReactNode;
  width?: string; }

export interface SortConfig { key: string;
  direction: 'asc' | 'desc'; }
...
export interface DashboardStats { total_users: number;
  active_users: number;
...
export interface RequestConfig { method: HTTPMethod;
...
export interface EnvironmentConfig { API_BASE_URL: string;
...
export interface FilterOption { key: string;
  label: string;
