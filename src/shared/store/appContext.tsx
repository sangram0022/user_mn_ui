...
interface AppState {
  ...
  // Loading states
  loading: { auth: boolean; dashboard: boolean; users: boolean; system: boolean };
...
const initialState: AppState = {
  ...
  loading: { auth: false, dashboard: false, users: false, system: false },

  errors: { auth: null, dashboard: null, users: null, system: null },

  cache: { lastUsersFetch: null, lastSystemHealthCheck: null, lastNotificationsFetch: null },
};
  ...
  useEffect(() => {
    try {
      ...
      if (persistedState) {
        ...
        if (parsed.sidebar) {
          dispatch({ type: 'SET_SIDEBAR_COLLAPSED', payload: parsed.sidebar.isCollapsed });
        }
      }
    } catch (error) {
...
export const useIsLoading = (keys?: (keyof AppState['loading'])[]): boolean => {
  const state = useAppState();
