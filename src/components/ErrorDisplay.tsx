  ...
  return (
    <div className={`${baseClasses[variant]} ${className}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className={messageClasses[variant]}>
          {error.userMessage}
        </div>
        ...
        {(error.supportText || error.supportUrl || error.correlationId) && (
          <div className={`mt-4 text-xs ${colors.text} opacity-80 space-y-1`}> 
            ...
            {error.correlationId && (
              <p>
                Reference ID:{' '}
                <code className="font-mono tracking-tight">{error.correlationId}</code>
              </p>
            )}
          </div>
        )}
      ...
      {onClose && variant !== 'banner' && (
        <button
          onClick={onClose}
          className={`flex-shrink-0 p-1 rounded-md ${colors.buttonSecondary} transition-colors duration-200`}
...
/**
 * Specialized Rate Limit Error Component
 ...
 * For temporary error notifications that auto-dismiss
 */
