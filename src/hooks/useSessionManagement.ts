...
interface SessionConfig { maxInactiveTime: number; // in milliseconds
  warningTime: number; // show warning before expiry
  checkInterval: number; // how often to check session }

const DEFAULT_CONFIG: SessionConfig = { maxInactiveTime: 30 * 60 * 1000, // 30 minutes
  warningTime: 5 * 60 * 1000, // 5 minutes before expiry
  checkInterval: 30 * 1000, // check every 30 seconds };

export const useSessionManagement = (config: Partial<SessionConfig> = {}) => { const { user, logout } = useAuth();
  ...
  // Update activity
  const updateActivity = useCallback(() => { if (sessionData && user) {
      const now = Date.now();
  ...
  const checkSession = useCallback(() => { if (!sessionData || !user) return;
    ...
    } else if (timeUntilExpiry <= sessionConfig.warningTime && !showWarning) { // Show warning
      setShowWarning(true);
    }
  }, [sessionData, user, logout, sessionConfig.warningTime, showWarning]);

  // Extend session
  const extendSession = useCallback(() => { updateActivity();
  }, [updateActivity]);
  ...
  // Activity listeners
  useEffect(() => { const activities = ['mousedown', 'mousemove', 'keypress', 'scroll', 'touchstart', 'click'];
    
    const handleActivity = () => {
      updateActivity();
    };

    activities.forEach(activity => { document.addEventListener(activity, handleActivity, true);
    });
  ...
  const remainingTime = useMemo(() => { if (!sessionData) return 0;
