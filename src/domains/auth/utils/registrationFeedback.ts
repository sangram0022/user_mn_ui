...
export interface RegistrationFeedback { title: string;
  subtitle: string;
  message: string;
  email: string;
...
export function buildRegistrationFeedback(
...
): RegistrationFeedback { const { email, verification_required, approval_required } = response;
  ...
  if (verification_required) { title = 'Check Your Email';
    ...
    nextSteps.push({ id: 'verify-email',
  ...
  if (approval_required) { highlights.push({
    ...
    nextSteps.push({ id: 'admin-approval',
  ...
  if (!verification_required && !approval_required) { redirectSeconds = 5;
    nextSteps.push({
      id: 'login',
