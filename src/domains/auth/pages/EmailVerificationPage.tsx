...
const EmailVerificationPage: React.FC = () => { const [searchParams] = useSearchParams();
  ...
  useEffect(() => { const verifyEmail = async () => {
      if (!token) {
        setStatus('error');
      ...
      try { const response = await apiClient.verifyEmail(token);
  ...
  useEffect(() => { if (status !== 'success') {
    ...
    if (countdown > 0) { const timer = window.setTimeout(() => {
