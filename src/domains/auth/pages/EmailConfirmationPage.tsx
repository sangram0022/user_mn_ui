...
const EmailConfirmationPage: React.FC = () => { const location = useLocation();
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => { setCountdown(prev => {
        if (prev <= 1) {
          navigate('/login', {
            state: {
  ...
  const handleResendEmail = async () => { if (!email || isResending) {
      return;
    }
    ...
    try { const response = await apiClient.resendVerification({ email });
      setResendMessage(
        response.message ?? 'Verification email has been resent successfully!'
      );
    } catch (error) { logger.error('Resend verification error:', undefined, { error  });
        ...
        <div className="bg-white shadow-xl sm:rounded-2xl border border-gray-100 p-8 sm:p-10">
          <div className="text-center">
            ...
            <div className="space-y-4">
              {resendMessage && (
                <div
                  className={`p-3 rounded-lg text-sm ${ resendMessage.includes('successfully')
