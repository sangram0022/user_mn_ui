...
export const AuthProvider: FC<AuthProviderProps> = ({ children }) => { const [user, setUser] = useState<UserProfile | null>(null);
  ...
  const checkAuthStatus = async () => { try {
    ...
    } catch (err) { logger.error('Auth check failed', err instanceof Error ? err : new Error(String(err)));
      // Clear invalid token
  ...
  const logout = async () => { try {
    ...
    } catch (err) { logger.warn('Logout API call failed', { error: String(err)  });
  ...
  const hasPermission = (permission: string): boolean => { if (!user) return false;
  ...
  const refreshProfile = async () => { try {
      setError(null);
      const userProfile = await apiClient.getUserProfile();
      setUser(userProfile);
    } catch (err) { setError(err instanceof Error ? err.message : 'Profile refresh failed');
      throw err;
    }
  };
