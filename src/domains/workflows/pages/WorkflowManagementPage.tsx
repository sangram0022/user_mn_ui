...
interface WorkflowStats { total_pending: number;
...
const WorkflowManagement: FC = () => { const { hasPermission, user } = useAuth();
  ...
  const loadWorkflows = useCallback(async () => {
    try {
      ...
      logger.info('🔄 Workflow: Loading workflows from backend...');
      ...
      logger.warn('⚠️ getPendingWorkflows API not implemented, using empty data');

      // Use empty data for now
      setWorkflows([]);
      logger.info('📝 Using empty workflow data - backend not available');
      setWorkflows([]);
      ...
      logger.error('❌ Workflow error:', undefined, { err  });
      setWorkflows([]);
    } finally { setIsLoading(false);
    }
  }, [filterStatus, filterType]);

  const loadStats = useCallback(async () => { try {
      logger.info('📊 Workflow: Loading workflow statistics...');
      
      // Mock data since API method doesn't exist
      logger.warn('⚠️ getWorkflowStats API not implemented, using mock data');

      // If backend fails, use empty stats
      logger.info('📊 Using empty workflow stats - backend not available');
      setStats({
        total_pending: 0,
        total_approved: 0,
        total_rejected: 0,
    ...
    } catch (err: unknown) { logger.error('❌ Workflow stats error:', undefined, { err  });
      setStats({ total_pending: 0,
        total_approved: 0,
        total_rejected: 0,
        pending_by_type: []
      });
  ...
  if (!hasPermission('workflow:read')) { return (
      <div style={{
        textAlign: 'center',
        padding: '2rem',
...
// Stat Card Component
const StatCard: FC<{ title: string;
  value: number;
  icon: string;
  color: string; }> = ({ title, value, icon }) => { return (
    <div style={{
      background: 'var(--background-secondary)',
      padding: '1.5rem',
      borderRadius: '12px',
...
const WorkflowCard: FC<{ workflow: WorkflowAction;
  index: number;
  onView: () => void;
  ...
  const getWorkflowIcon = (type: string) => { switch (type) {
...
const WorkflowModal: FC<{ workflow: WorkflowAction;
  onAction: (id: number, action: 'approve' | 'reject', justification?: string) => void;
  onClose: () => void;
  hasApprovalPermission: boolean;
  actionLoading: string | null;
  ...
  const handleAction = (action: 'approve' | 'reject') => { if (action === 'reject' && !justification.trim()) {
