...
const StatusIndicator: FC<{ status: string; size?: 'sm' | 'md' | 'lg' }> = ({
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      ...
      case 'degraded':
        return { color: 'bg-yellow-500', icon: AlertTriangle };
      case 'down':
        return { color: 'bg-red-500', icon: XCircle };
      case 'maintenance':
        return { color: 'bg-blue-500', icon: Settings };
      default:
  ...
  return <div className={`${getSizeClasses(size)} ${color} rounded-full`} />;
};
...
const MetricCard: FC<{
  ...
  return (
    <div className={`p-6 rounded-lg border ${getStatusColor(status)}`}>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <span
              className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusColorClass(service.status)}`}
            >
...
const AlertCard: FC<{
  ...
  const { color, badge, icon: Icon } = getSeverityConfig(alert.severity);

  return (
    <div className={`p-4 rounded-lg border ${color} ${alert.acknowledged ? 'opacity-75' : ''}`}>
      <div className="flex items-start justify-between">
        <div className="flex items-start space-x-3">
          <Icon className="w-5 h-5 mt-0.5 text-current" />
