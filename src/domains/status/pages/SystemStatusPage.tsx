...
const SystemStatus: FC = () => { const { user, hasPermission } = useAuth();
  ...
  const updateCheck = useCallback((name: string, status: SystemCheck['status'], message: string, details?: string) => { setChecks((prev) => prev.map((check) => (check.name === name ? { ...check, status, message, details } : check)));
  }, []);

  const runSystemChecks = useCallback(async () => { setIsRunning(true);
    ...
    if (user) {
      ...
      updateCheck('Authentication', 'success', `Authenticated as ${user.email}`, `Role: ${roleName}, Active: ${user.is_active}`);
    } else { updateCheck('Authentication', 'error', 'Not authenticated', 'Please log in');
    }
    ...
    } else { updateCheck('User Permissions', 'error', 'No permissions available', 'Not authenticated');
    }
    ...
    try {
      ...
      if (healthResponse.ok) {
        ...
        updateCheck('Backend Connection', 'success', `Backend healthy (${healthData.api_version})`, `Status: ${healthData.status}`);
      } else {
        updateCheck('Backend Connection', 'error', `Backend returned ${healthResponse.status}`, await healthResponse.text());
      }
    ...
    } catch (error) { updateCheck('Roles API', 'error', 'Roles API failed', error instanceof Error ? error.message : 'Unknown error');
    }

    const dashboardLinks = ['/users', '/analytics', '/workflows', '/security', '/settings', '/reports'];
    dashboardLinks.filter((link) => { try {
        return window.location.pathname !== link;
      } catch { return false;
      }
    });

    updateCheck('Navigation Links', 'success', `${dashboardLinks.length} dashboard links configured`, `Links: ${dashboardLinks.join(', ')}`);

    setIsRunning(false);
  }, [user, updateCheck, hasPermission]);

  useEffect(() => { runSystemChecks();
  }, [user, runSystemChecks]);

  const getStatusIcon = (status: SystemCheck['status']) => { switch (status) {
      case 'checking':
  ...
  const getStatusColor = (status: SystemCheck['status']) => { switch (status) {
      case 'checking':
